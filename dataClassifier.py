...
def enhancedFeatureExtractorDigit(datum):
    ...
    # check if there is a circle in the datum.
    # it does this by going through every single pixel that is off,
    # then going north, east, south, west, and all the directions inbetween,
    # and checking whether or not there is a pixel on.
    # if there is a pixel on in all eight directions, there is a circle.
...
def enhancedPacmanFeatures(state, action):
    ...
    for p in suc.getGhostPositions():
        #check if pacman will die next turn
        if(p == nextMan):
